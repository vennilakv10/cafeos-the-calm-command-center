...
function Branches() { return <div className="page-stack"><section className="branch-total"><div><span>All branches · Today</span><strong>₹82,460</strong><p><ArrowUpRight/> 10.8% vs yesterday</p></div><Metric label="Total orders" value="312" note="Across 2 branches"/><Metric label="Net profit" value="₹29,640" note="36% of sales"/><Metric label="Alerts" value="6" note="4 in Kochi"/></section><div className="branch-cards">{[["Kochi","Open · closes 11 PM","₹48,920","184","61%","4"],["Dubai","Open · closes 12 AM","AED 1,480","128","64%","2"]].map((b,i)=><article key={b[0] ?? "branch"}><header><span className="branch-big-icon"><Store/></span><div><h2>{b[0] ?? "Branch"}</h2><Status tone="success">{b[1] ?? "Open"}</Status></div><Button variant="secondary" size="sm">Open branch<ArrowRight/></Button></header><div><Metric label="Sales today" value={b[2] ?? "—"}/><Metric label="Orders" value={b[3] ?? "—"}/><Metric label="Gross margin" value={b[4] ?? "—"}/></div><footer><AlertTriangle/><span><b>{b[5] ?? "0"} items need attention</b>{i===0?"Chicken and coffee beans are low":"Two staff shifts need review"}</span><ArrowRight/></footer></article>)}</div><section><SectionHeading title="Branch comparison"/><div className="comparison"><div><b>Kochi</b><span>Sales</span><i><em style={{width:"74%"}}/></i><strong>₹48,920</strong></div><div><b>Dubai</b><span>Sales</span><i><em style={{width:"56%"}}/></i><strong>AED 1,480</strong></div></div></section></div>; }
